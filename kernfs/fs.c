			...
			case L_TYPE_INODE_UPDATE: {
				...
				case L_TYPE_FILE: {
					...
					if (!item) {
						...
						// avoid case of DIR_DEL, DIR_RENAME, DIR_ADD 
						// if(loghdr->type[i] == L_TYPE_DIR_ADD)
						// 	mlfs_debug("[DIR_ADD] found in FILE %u\n", item->key.inum);
						...
						list_add_tail(&item->list, &replay_list->head);
						mlfs_debug("[FILE] inum %u (ver %u) - NOT in Hash \n",
								item->key.inum, item->key.ver);
					}
					else{
						mlfs_debug("[FILE] inum %u (ver %u) - In Hash \n",
								item->key.inum, item->key.ver);
					}
...
static void digest_log_from_replay_list(uint8_t from_dev, int libfs_id, struct replay_list *replay_list)
{
	...
	list_for_each_safe(l, tmp ,&replay_list->head){
		...
		if(*node_type == NTYPE_I){
			...
			// HASH_ITER(hh, replay_list->i_digest_hash, i_item, t){
