...
#define thread1_off (unsigned long)(1<<30)  // 1GB
#define thread2_off (2UL)*(unsigned long)(1<<30) // 2GB
...
void io_bench::prepare(void)
{
	...
	switch (id)
	{
		...
		case 1:{
			off_th = file_size_bytes;
			break;
		}
		case 2:{
			off_th = 2UL * file_size_bytes;
			break;
		}
...
int main(int argc, char *argv[])
{
    ...
    // for(unsigned long k = 0; k < (3UL)*(1<<30) ; k += 4096UL){
    //     write(fd, read_buf, 4096UL);
    // }
    for(unsigned long k = 0; k < (3UL)*(thread1_off) ; k += 4096UL){
        write(fd, read_buf, 4096UL);
    }
    fstat(fd, &stat);
	printf("filesize %lu tid: %d \n", stat.st_size, 1);
    printf("main thread creates file fd: %d \n", fd);
...
begin:
	...
	*/
	...
	lseek(fd, 0 ,SEEK_SET);
	for (unsigned long i = 0; i < file_size_bytes; i += 4096UL) {
		if(read(fd, read_buf, 4096UL) < 0)
			exit(-1);
		for(int j=0; j < 4096; j++){
			if(read_buf[j] != '1' + (char)0){
				
			}
		}
	}

    printf("Starting qualifying ... 1\n");
    lseek(fd, file_size_bytes ,SEEK_SET);
	for (unsigned long i = 0; i < file_size_bytes; i += 4096UL) {
		if(read(fd, read_buf, 4096UL) < 0)
			exit(-1);
		for(int j=0; j < 4096; j++){
			if(read_buf[j] != '1' + (char)1){
				exit(-1);
                break;
			}
		}
	}
    printf("Starting qualifying ... 2\n");
    lseek(fd, 2UL * file_size_bytes ,SEEK_SET);
	for (unsigned long i = 0; i < file_size_bytes; i += 4096UL) {
		if(read(fd, read_buf, 4096UL) < 0)
			exit(-1);
		for(int j=0; j < 4096; j++){
			if(read_buf[j] != '1' + (char)2){
				exit(-1);
                break;
			}
		}
