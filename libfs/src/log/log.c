...
#endif
	// This part digests when Libfs is terminated.
...
inline addr_t log_alloc(uint32_t nr_blocks)
{
	...
	if (!g_fs_log->digesting) {
		...
		if (enable_perf_stats)
			g_perf_stats.lease_revoke_wait_tsc += (asm_rdtscp() - help_tsc_tmp);
	}
	...
	// TODO: Before allocate the new blocks, check whether it extends the g_fs_log->stark_blk
	// if, it extends digesting is not done or something goes wrong.
...
retry:
	if (g_fs_log->avail_version > g_fs_log->start_version) {
		if (g_fs_log->start_blk - g_fs_log->next_avail_header
			...
			m_barrier();
			printf("synchronous digesting is on going!\n");
			wait_on_digesting();
		}
	}
...
//disabling coalescing
	...
	// Handling large (possibly multi-block) write.
		...
		mlfs_debug("inum %u offset %lu size %u @ blockno %lu (aligned)\n",
...
static void commit_log(void)
{
	...
	if (loghdr->n > 0) {
		...
		if(!loghdr_meta->is_hdr_locked && loghdr->n == 1 &&
			...
			//FIXME: this currently does not support multi-th
...
int make_digest_request_async(int percent)
{
	...
	if (!g_fs_log->digesting && atomic_load(&g_log_sb->n_digest) > 0) {
		set_digesting(&checker);
		//mlfs_debug("Send digest command: %s\n", cmd_buf);
		//ret = write(g_fs_log->digest_fd[1], cmd_buf, MAX_CMD_BUF);
		mlfs_printf("%s","make_digest_request_sync\n");
		if(checker){
			mlfs_printf("%s","captured by checker\n");
			return -EBUSY;
		}
