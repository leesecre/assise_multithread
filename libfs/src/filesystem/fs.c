...
pthread_rwlock_t *g_dir_entry_rwlock;
...
void shutdown_fs(void)
{
	...
	mlfs_printf("TID;: %ld shutdown here\n", get_tid());
	...
	// pthread_spin_unlock(&unlink_lock);
...
static void locks_init(void)
{
	...
	g_dir_entry_rwlock =  (pthread_rwlock_t *)mlfs_zalloc(sizeof(pthread_rwlock_t));
	...
	pthread_rwlock_init(g_dir_entry_rwlock, &rwlattr);
}
...
int itrunc(struct inode *ip, offset_t length)
{
	...
	if (length == 0) {
		...
		for (size = ip->size; size >= length ; size -= g_block_size_bytes) {
		...
		assert(0); // TODO: for real truncates.
...
*/
	...
	mlfs_printf("□[TID: %ld]read stats: inode[inum %u isize %lu] size_small %lu size_prepended %lu size_aligned %lu size_appended %lu\n",
			get_tid(), ip->inum, ip->size, size_small, size_prepended, size_aligned, size_appended);
	...
	mlfs_printf("■[TID: %ld]finishing read. iodone: %d\n", get_tid(), ret);
...
int add_to_log(struct inode *ip, uint8_t *data, offset_t off, uint32_t size, uint8_t ltype)
{
	...
	//[DISAGGREGATE]
	// fast persist_log_file for DIR_ADD
	// if (ltype == L_TYPE_DIR_ADD){
	...
	// 	if(fc_block){
	// 		if(!check_log_invalidation(fc_block)){
	// 			assert(0);
	// 		}
	// 	}
	// 	if(!fc_block){
	// 		fc_block = fcache_alloc_add(ip, key);
	// 	}
		
	// 	fc_block->log_version = g_fs_log->avail_version;

	// 	fcache_log_replace(fc_block, , , g_fs_log->avail_version);
