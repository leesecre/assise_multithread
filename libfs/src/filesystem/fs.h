...
extern pthread_rwlock_t *g_debug_rwlock;
extern pthread_rwlock_t *g_dir_entry_rwlock;
		...
		const char *_name, offset_t *offset)
{
	...
	// if (!kh_exist(dir_inode->de_cache_hash, k)){
		*offset = 0;
...
#ifdef MLFS_MULTITHD
	if(_offset == dir_inode->size - sizeof(struct mlfs_dirent))
		dir_inode->de_cache_last = _dirent_data;
#endif	
...
static inline int de_cache_del(struct inode *dir_inode, const char *_name)
{
	...
	// if(!strcmp(_name, dir_inode->de_cache_last->name))
	// 	dir_inode->de_cache_last = NULL;
	...
	// if (k != kh_end(dir_inode->de_cache_hash)) {
		...
		mlfs_printf("[de_cache not exists TT ][TID: %ld] %s \n", get_tid(), _name);
	}
	return 0;
}
static inline int de_cache_findbyoff(struct inode *inode, offset_t offset){
	khiter_t k;
	struct dirent_data *_dirent_data;
	int ret = 0;

	pthread_rwlock_rdlock(&inode->de_cache_rwlock);
	for (k = kh_begin(inode->de_cache_hash); 
		...
		if (kh_exist(inode->de_cache_hash, k)) {
			...
			if(_dirent_data->offset == offset){
				inode->de_cache_last = _dirent_data;
				ret = 1;
				break;
			}
		}
	}
	pthread_rwlock_unlock(&inode->de_cache_rwlock);
	return ret;
}
