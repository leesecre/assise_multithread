...
int posix_init = 0;
pthread_spinlock_t unlink_lock;
void posix_lock_init(){
	pthread_spin_init(&unlink_lock, PTHREAD_PROCESS_SHARED);
}
...
int mlfs_posix_open(char *path, int flags, uint16_t mode)
{
	...
	//start_log_tx();
	// mlfs_printf("[tid: %ld][POSIX] open(path=%s), O_CREATE:%d\n", get_tid(), path, flags & O_CREAT);
...
int mlfs_posix_read(int fd, uint8_t *buf, int count)
{
	...
	mlfs_printf("[TID: %ld][POSIX] read(fd=%d, size=%d)\n",get_tid(), fd, count);
	...
	pthread_rwlock_unlock(&f->rwlock);
	mlfs_printf("[TID: %ld][POSIX] read done(fd=%d, size=%d)\n",get_tid(), fd, count);
	mlfs_posix("[POSIX] read done(fd=%d, size=%d)\n", fd, count);
	return ret;
}
...
{
	struct inode *inode;
	// mlfs_printf("[tid: %ld][POSIX] stat(name=%s)\n", get_tid(), filename);
...
int mlfs_posix_unlink(const char *filename)
{
	...
	// pthread_spin_lock(&unlink_lock);
	// mlfs_printf("[tid: %ld] name=%s)", get_tid(), filename);
